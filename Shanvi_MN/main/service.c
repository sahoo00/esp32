...
#define GATTS_TABLE_TAG "Shanvi MN Service"
...
#define SAMPLE_DEVICE_NAME          "Sh_MN"
...
static uint8_t raw_adv_data[] = {
        ...
        0x03, 0x03, 0x02, 0x00,
        /* device name */
        0x0f, 0x09, 'S', 'h', '_', 'M', 'N', ' ', 'R', 'A', 'W', ' ', 'D','E', 'M', 'O'
};
...
static struct gatts_profile_inst mn_profile_tab[PROFILE_NUM] = {
    [PROFILE_APP_IDX] = {
...
/* Service */
static const uint16_t MN_SAFETY_SERVICE       	= 0x0002;
static const uint16_t MNCH_SAFETY_ALERT       	= 0x0010;
static const uint16_t MNCH_ALERT_STATUS       	= 0x0011;
static const uint16_t MN_CHAR_WIFI_SSID       	= 0x0012;
static const uint16_t MN_CHAR_WIFI_PASS       	= 0x0013;
static const uint16_t MN_CHAR_WIFI_CONN       	= 0x0014;
...
static const uint8_t measurement_ccc[2]      	   = {0x00, 0x00};
...
static const esp_gatts_attr_db_t gatt_db[HRS_IDX_NB] =
{
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_client_config_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
			      sizeof(uint16_t), sizeof(measurement_ccc), (uint8_t *)measurement_ccc}},
    ...
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if (mn_handle_table[IDX_CHAR_VAL_C] == param->write.handle) {
                	set_ssid((char*)param->write.value, param->write.len);
                }
                if (mn_handle_table[IDX_CHAR_VAL_D] == param->write.handle) {
                	set_pass((char*)param->write.value, param->write.len);
                }
                if (mn_handle_table[IDX_CHAR_VAL_E] == param->write.handle) {
                	connect_wifi();
                }
                if (mn_handle_table[IDX_CHAR_CFG_E] == param->write.handle && param->write.len == 2){
                    uint16_t descr_value = param->write.value[1]<<8 | param->write.value[0];
                    conn_enable = 0x00;
                    if (descr_value == 0x0001){
                        ...
                        ESP_LOGE(GATTS_TABLE_TAG, "CONN unknown descr value");
                        esp_log_buffer_hex(GATTS_TABLE_TAG, param->write.value, param->write.len);
                    }
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT:{
            if (param->add_attr_tab.status != ESP_GATT_OK){
                ...
                memcpy(mn_handle_table, param->add_attr_tab.handles, sizeof(mn_handle_table));
                esp_ble_gatts_start_service(mn_handle_table[IDX_SVC]);
            }
            break;
        }
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            mn_profile_tab[PROFILE_APP_IDX].gatts_if = gatts_if;
            mn_profile_tab[PROFILE_APP_IDX].conn_id = param->write.conn_id;
        } else {
    ...
    do {
        ...
        for (idx = 0; idx < PROFILE_NUM; idx++) {
            ...
            if (gatts_if == ESP_GATT_IF_NONE || gatts_if == mn_profile_tab[idx].gatts_if) {
                if (mn_profile_tab[idx].gatts_cb) {
                    mn_profile_tab[idx].gatts_cb(event, gatts_if, param);
                }
            }
        }
    } while (0);
...
void notify_connection(uint8_t *data, int len) {
	if (conn_enable != 0) {
		ESP_LOGI(GATTS_TABLE_TAG, "CONN notify");
		//the size of notify_data[] need less than MTU size
		esp_ble_gatts_send_indicate(mn_profile_tab[PROFILE_APP_IDX].gatts_if,
